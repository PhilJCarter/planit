...
def load_G2_1(snap, fname, headonly=False, thermo=False, compress=False, mats=[402,400]):
    """
...
def load_hdf5(snap, fname, headonly=False, recenter=True, thermo=False, compress=False, debug=False):
    """
...
def write_G2_1(snap, fname):
    ...
    f.write(struct.pack(count3 + 'f',
                         *npy.array(snap.pos).reshape((3*snap.N))))
    f.write(struct.pack('i', snap.N))  #SKIP

    f.write(struct.pack('i', snap.N))  #SKIP
    f.write(struct.pack(count3 + 'f',
                         *npy.array(snap.vel).reshape((3*snap.N))))
    f.write(struct.pack('i', snap.N))  #SKIP
